// Versão mais longa (15s) do anúncio pago pro TikTok — mesma base visual e
// timing de gancho de gera-video-anuncio-tiktok.mjs (9,5s), com uma segunda
// leva de benefícios ("E tem mais:") antes do CTA, pra cobrir mais do app
// já que sobra tempo de vídeo. Peça isolada, não entra na fila automática
// (mesmo motivo dos outros anúncios).
//
// Uso:  node finn-social/ads/gera-video-anuncio-tiktok-15s.mjs
...
const W = 1080, H = 1920;
// O Playwright não grava o webm em 1:1 exato com o tempo real de espera —
// o vídeo fica uns 10-14% "mais lento" que o wall-clock, e o efeito cresce
// quanto mais tarde é o delay (medido empiricamente: um elemento com
// animation-delay de 11,5s só aparecia de verdade perto de 13,3-13,4s no
// vídeo final, quase sem sobra de tempo antes do corte). Os delays dos
// "beats" tardios (2ª leva de benefícios, CTA, rodapé) abaixo já são os
// valores AJUSTADOS pra compensar isso — não são os segundos "de relógio"
// que você imaginaria lendo isolado. DURACAO um pouco maior que 15 pra
...
const html = `<!doctype html><meta charset="utf-8"><style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .chipsRow{position:absolute;left:96px;width:888px;display:flex;flex-wrap:wrap;gap:14px}
  ...
  /* ── mockup do app (mesmo visual do anúncio estático) ── */
  ...
  .mais{position:absolute;left:96px;top:1390px;width:888px;opacity:0;
    font-size:34px;font-weight:800;color:${LARANJA}}

  .cta{position:absolute;left:96px;top:1600px;width:888px;opacity:0;display:flex;justify-content:center}
  ...
  .rodape{position:absolute;left:96px;top:1750px;width:888px;text-align:center;opacity:0;
...
<!-- Gancho quase instantâneo (badge + h1), timing validado nos vídeos
...
<div class="beat fadeUp" style="top:620px;animation-delay:1s">
  <p style="font-size:36px;line-height:1.5;font-weight:400;color:${CINZA_ESCURO}">${esc(sub)}</p>
</div>

<div class="chipsRow" style="top:800px">
  ${chips1.map((c, i) => `<div class="chip fadeUp" style="animation-delay:${(1.8 + i * 0.15).toFixed(2)}s">${esc(c)}</div>`).join('')}
</div>
...
<!-- Segunda leva de benefícios: a tela principal (gancho + mockup) fica
     parada de ~3,4s a 7,5s pra dar tempo de assimilar, só depois entra o
     "e tem mais" — vídeo mais longo dá espaço pra falar de mais recursos
     sem apressar o gancho nem o CTA. -->
<div class="mais fadeUp" style="animation-delay:6.6s">${esc(maisLabel)}</div>
<div class="chipsRow" style="top:1450px">
  ${chips2.map((c, i) => `<div class="chip fadeUp" style="animation-delay:${(6.8 + i * 0.15).toFixed(2)}s">${esc(c)}</div>`).join('')}
</div>

<div class="cta pop" style="animation-delay:10.2s">
