// Vídeo de anúncio pro TikTok Ads Manager, pra campanha de TRÁFEGO/
// CONVERSÃO (não o botão simplificado "Promover" dentro do app, que só
// deixa escolher objetivos de engajamento — ver a conversa que motivou
// este arquivo: uma promoção anterior gastou R$28 em "mais curtidas e
// comentários" e trouxe 564 curtidas mas só 1 visualização de perfil,
// porque não era isso que o algoritmo estava otimizando).
//
// Duas mudanças em relação a gera-video-anuncio-tiktok.mjs (a versão
// genérica de 9,5s):
//
// 1. ZONA SEGURA DO TIKTOK: o CTA e o rodapé daquela versão ficavam entre
//    top:1500 e ~1690px (de 1920px de altura) — dentro ou muito perto da
//    faixa inferior que o próprio app do TikTok cobre com legenda, ícone de
//    música, e os botões de curtir/comentar/compartilhar/perfil (a faixa
//    reservada costuma ficar pelos últimos ~250-300px). Nada garante que
//    esse texto estivesse sendo visto de verdade. Aqui todo o conteúdo
//    termina em ~1600px, com ~320px de sobra embaixo.
//
// 2. CTA mais cedo (4,2s em vez de 5s): os dados reais da promoção anterior
//    mostraram queda de atenção muito forte depois dos 2-6s (18,3% chegam
//    aos 2s, só 3,4% aos 6s) — então o CTA precisa aparecer o quanto antes
//    depois do mockup assentar, pra ganhar mais segundos de exposição
//    dentro da janela de atenção real que o vídeo tem.
//
// Uso:  node finn-social/ads/gera-video-anuncio-tiktok-trafego.mjs
...
const html = `<!doctype html><meta charset="utf-8"><style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .chipsRow{position:absolute;left:96px;top:770px;width:888px;display:flex;flex-wrap:wrap;gap:14px}
  ...
  /* ── mockup do app (mesmo visual dos outros anúncios) ── */
  .mock{position:absolute;left:114px;top:920px;width:852px;height:404px;opacity:0;
  ...
  .meta{position:absolute;left:824px;top:866px;width:242px;opacity:0;background:#fff;border-radius:20px;
  ...
  /* CTA + rodapé terminam em ~1600px (de 1920) — fora da faixa que o
     TikTok cobre com legenda/música/botões de engajamento. */
...
<div class="cta pop" style="animation-delay:4.2s">
  <div class="cta-btn">${esc(ctaTxt)}</div>
</div>
<div class="rodape fadeUp" style="animation-delay:4.4s">${esc(rodapeTxt)}</div>
`;
...
const saida = path.join(DIR, 'anuncio-video-tiktok-trafego.mp4');
// '-ss 0.15' pula o frame em branco inicial do Playwright. '-crf 16 -preset
// slow -tune animation': qualidade alta (ver gera-video-anuncio-tiktok-15s.mjs).
// '-vf scale=2160:3840:flags=lanczos': exporta em 4K vertical — gravar
// direto em 4K quebra a gravação do Playwright (bug real, documentado em
// gera-video-anuncio-tiktok-15s.mjs), então a gravação continua em
// 1080x1920 e sobe de resolução só na exportação.
